...
async fn test_tool_execution_via_server() {
    ...
    let result1 = server.thinking(Parameters(input1)).await.unwrap();
    ...
    let result2 = server.thinking(Parameters(input2)).await.unwrap();
...
async fn test_tool_error_via_server() {
    ...
    let result = server.thinking(Parameters(invalid_input)).await.unwrap();
    assert_eq!(result.is_error, Some(true));
    let text = result.content[0].as_text().unwrap();
    assert!(text.text.contains("Thinking Error:"));
...
async fn test_server_info_and_instructions() {
    ...
    assert_eq!(info.server_info.name, "seq");
    assert_eq!(info.server_info.version, "0.2.0");
    assert!(info.instructions.is_some());
    let instructions = info.instructions.unwrap();
    assert!(instructions.contains("Seq Server"));
    assert!(instructions.contains("thinking"));
